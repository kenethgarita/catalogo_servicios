...
export async function ObtenerSolicitudServicioPorId(id_solicitud_servicio) {
    ...
    const result = await pool.request()
        ...
        .query("SELECT * FROM Solicitud_servicio WHERE id_solicitud_servicio = @id_solicitud_servicio");
    return result.recordset[0];
}

// Actualizar relación
export async function ActualizarSolicitudServicio(id_solicitud_servicio, { id_solicitud, id_servicio }) {
    const pool = await connectDB();
    const result = await pool.request()
        .input("id_solicitud_servicio", sql.Int, id_solicitud_servicio)
        .input("id_solicitud", sql.Int, id_solicitud)
...
export async function EliminarSolicitudServicio(id_solicitud_servicio) {
    ...
    const result = await pool.request()
        .input("id_solicitud_servicio", sql.Int, id_solicitud_servicio)
        .query("DELETE FROM Solicitud_servicio WHERE id_solicitud_servicio = @id_solicitud_servicio");
    return result.rowsAffected[0];
}
