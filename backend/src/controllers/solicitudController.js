...
export const CreaSolicitud = async (req, res) => {
    const { id_usuario, detalles_solicitud, estado_solicitud } = req.body;
    if (!id_usuario || !detalles_solicitud) {
        return res.status(400).json({ error: "Faltan campos obligatorios" });
    }
    try {
        const solicitud = await CrearSolicitud({ id_usuario, detalles_solicitud, estado_solicitud });
...
export const EditarSolicitud = async (req, res) => {
    const { id_solicitud } = req.params;
    try {
        const filasActualizadas = await ActualizarSolicitud(id_solicitud, req.body);
