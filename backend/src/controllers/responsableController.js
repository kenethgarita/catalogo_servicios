import { CrearResponsable, ObtenerResponsables, ObtenerResponsablePorId, ActualizarResponsable, EliminarResponsable } from "../models/responsableModel.js"

export const CreaResponsable = async (req, res) => {
    const { id_usuario, id_servicio } = req.body;
    if (!id_usuario || !id_servicio) return res.status(400).json({ error: "Faltan campos obligatorios" });
    try {
...
export const EditarResponsable = async (req, res) => {
    const { id_responsable } = req.params;
    try {
        const filasActualizadas = await ActualizarResponsable(id_responsable, req.body);
        if (filasActualizadas === 0) return res.status(404).json({ error: "Responsable no encontrado" });
...
export const BorrarResponsable = async (req, res) => {
    ...
    try {
        ...
        if (filasEliminadas === 0) return res.status(404).json({ error: "Responsable no encontrado" });
