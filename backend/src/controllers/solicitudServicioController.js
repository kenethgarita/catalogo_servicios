import { CrearSolicitudServicio, ObtenerSolicitudServicios, ObtenerSolicitudServicioPorId, ActualizarSolicitudServicio, EliminarSolicitudServicio } from "../models/solicitudServicioModel.js";

export const CreaSolicitudServicio = async (req, res) => {
    const { id_solicitud, id_servicio } = req.body;
    if (!id_solicitud || !id_servicio) return res.status(400).json({ error: "Faltan campos obligatorios" });
    try {
        const relacion = await CrearSolicitudServicio({ id_solicitud, id_servicio });
...
export const EditarSolicitudServicio = async (req, res) => {
    const { id_solicitud_servicio } = req.params;
    try {
        const filasActualizadas = await ActualizarSolicitudServicio(id_solicitud_servicio, req.body);
        if (filasActualizadas === 0) return res.status(404).json({ error: "Relación no encontrada" });
...
export const BorrarSolicitudServicio = async (req, res) => {
    const { id_solicitud_servicio } = req.params;
    try {
        const filasEliminadas = await EliminarSolicitudServicio(id_solicitud_servicio);
        if (filasEliminadas === 0) return res.status(404).json({ error: "Relación no encontrada" });
